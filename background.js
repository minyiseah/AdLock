...
async function generateContextualContent(pageTitle, pageUrl, requestedType) {
  ...
  const now = Date.now();
  const prompt = `You are a sarcastic productivity enforcer. The user is procrastinating on "${pageTitle || 'a mystery site'}" (${pageUrl}).
  ...
  try {
    const response = await fetch('https://api.openai.com/v1/chat/completions', {
      ...
      body: JSON.stringify({
        ...
        response_format: {
          ...
          json_schema: {
            ...
            schema: {
              ...
              properties: {
                type: { type: 'string', enum: TYPE_SEQUENCE },
                text: { type: 'string' }
              },
              required: ['type', 'text']
            }
        ...
        messages: [
          { role: 'system', content: 'You are a sarcastic productivity bot that only responds with raw minified JSON.' },
    ...
    if (!message) {
      console.error('OpenAI API failure - No choices returned.', data);
      return null;
    }

    let text = message.trim().replace(/```json/g, '').replace(/```/g, '').trim();
...
async function generateStudyQuizQuestions(fileMeta, textPreview, imageDataUrl) {
  const OPENAI_API_KEY = CONFIG?.OPENAI_API_KEY;
  if (!OPENAI_API_KEY) throw new Error('Missing OPENAI_API_KEY in config.js');
  if (!fileMeta) throw new Error('Missing study file metadata.');
  ...
  const userContent = [
    {
      type: 'text',
      text: 'Use the study material below to craft exactly three multiple-choice quiz questions (A-D options) with their correct answers and concise explanations.'
    },
    {
      type: 'text',
  ...
  if (cleanedText) {
    userContent.push({ type: 'text', text: `Study excerpt:\n${cleanedText}` });
  }
  ...
  const response = await fetch('https://api.openai.com/v1/chat/completions', {
    ...
    body: JSON.stringify({
      ...
      messages: [
        { role: 'system', content: 'You are a strict tutor who only outputs raw minified JSON.' },
  ...
  const data = await response.json();
  const message = data.choices?.[0]?.message?.content;
  if (!message) throw new Error('OpenAI response missing quiz content.');
...
function checkAndRoast(tabId, tab) {
  ...
  chrome.storage.sync.get(['blacklist', 'sessionActive', 'sessionEnd'], async (result) => {
    ...
    if (!isBlacklisted || !contextKey) return;
